...
interactive:
    ...
    # Select program type
    program=$(go tool gum choose "bitcoind" "lnd")
    
    if [ "$program" = "bitcoind" ]; then
        # Bitcoin CLI commands
        command=$(go tool gum choose "getinfo" "generate" "generateemptyblock" "sendtoaddress" "getbalance" "getnewaddress" "getblockcount" "custom")
        ...
        case $command in
            "getinfo")
                ...
                ;;
            "generate")
                blocks=$(go tool gum input --placeholder "Number of blocks to generate")
            ...
            "generateemptyblock")
                ...
                ;;
            "sendtoaddress")
                address=$(go tool gum input --placeholder "Destination address")
                amount=$(go tool gum input --placeholder "Amount to send")
            ...
            "getblockcount")
                ...
                ;;
            "custom")
                cmd=$(go tool gum input --placeholder "Enter custom bitcoin-cli command")
        ...
        # LND commands
        command=$(go tool gum choose "getinfo" "addinvoice" "payinvoice" "listchannels" "openchannel" "closechannel" "walletbalance" "channelbalance" "newaddress" "custom")
        ...
        case $command in
            "getinfo")
                ...
                ;;
            "addinvoice")
                amount=$(go tool gum input --placeholder "Invoice amount in satoshis")
                just addinvoice $amount
                ;;
            "payinvoice")
                invoice=$(go tool gum input --placeholder "Payment request/invoice")
            ...
            "listchannels")
                ...
                ;;
            "openchannel")
                pubkey=$(go tool gum input --placeholder "Node public key")
                amount=$(go tool gum input --placeholder "Channel amount in satoshis")
                just lncli openchannel --node_key $pubkey --local_amt $amount
                ;;
            "closechannel")
                funding_txid=$(go tool gum input --placeholder "Funding transaction ID")
                output_index=$(go tool gum input --placeholder "Output index")
            ...
            "newaddress")
                ...
                ;;
            "custom")
                cmd=$(go tool gum input --placeholder "Enter custom lncli command")
                just lncli $cmd
                ;;
        esac
    fi
