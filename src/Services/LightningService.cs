...
namespace FundsManager.Services
{
    ...
    public interface ILightningService
    {
        ...
        /// </summary>
        /// <returns></returns>
        [AutomaticRetry(LogEvents = true, Attempts = int.MaxValue)]
    ...
    public class LightningService : ILightningService
    {
        ...
        private void DeleteOldChannelAcceptorJobs()
        {
            ...
            var processingJobs = api.ProcessingJobs(0, int.MaxValue)
                .Where(x => x.Value.Job.ToString() == $"{nameof(ILightningService)}.{nameof(ChannelAcceptorJob)}" ||
                            x.Value.Job.ToString() == $"{nameof(ILightningService)}.{nameof(ProcessNodeChannelAcceptorJob)}")
        ...
        public async Task SweepNodeWalletJob(int managedNodeId)
        {
            ...
            var requiredAnchorChannelClosingAmount = long.Parse(Environment.GetEnvironmentVariable("ANCHOR_CLOSINGS_MINIMUM_SATS") ?? throw new InvalidOperationException()); // Check https://github.com/lightningnetwork/lnd/issues/6505#issuecomment-1120364460 to understand, we need 100K+ to support anchor channel closings
